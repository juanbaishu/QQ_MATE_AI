            ...
            try:
                ...
                if segment_type == "text":
                    ...
                    chat_history.append({"role": "user", "content": message_text})         # "user"是API协议中规定的关键字，代表用户说的话
                                    # 呼叫大模型思考回复
                    ai_response = client.chat.completions.create (         # 调用大模型 API，让它根据 chat_history 里的内容生成回复。
                ...
                # 图片信息处理
                    ...
                    message_image_url = segment.get("data", {}).get("url")
                    chat_history.append({"role": "user", "content": [{"type": "image_url", 
                                                                      "image_url":{"url": message_image_url} 
