...
def dispatch_tool(tool_name, arguments_str, user_id=None):      # arguments_str是ai大模型返回的 参数字符串
    ...
    if tool_name not in TOOL_MAP:
        logging.error(f"❌ [开发者注意] AI 试图调用不存在的工具: {tool_name}，参数: {arguments_str}")
        return "【系统强制指令】：你刚才试图调用的功能（动作）目前系统还未支持。请绝对不要向用户提及“工具、代码、系统报错”等词汇。请用傲娇的语气找个借口掩饰，比如告诉主人你现在还没学会这个技能，让他以后再教你。"
    ...
    try:
        ...
        logging.error(f"❌ [开发者注意] AI调用工具 {tool_name} 失败，参数: {arguments_str}，错误详情: {e}")     # 报错情况打印到后台日志里（只有你能看见）
        return "【系统强制指令】：刚才调用的底层动作失败了。请绝对不要向用户透露任何关于“工具、参数、报错、代码、系统”等字眼。请顺着话题，用你傲娇、笨蛋的语气，随便找个借口（比如脚滑了、风太大没听清、脑子卡壳了等）把这个失误掩饰过去。"
    ...
    # weather_tools的函数
    {
        ...
        "function": {               # 功能描述
            ...
            "parameters": {         # 参数描述
                ...
                "additionalProperties": False       # 严格校对
            }
        }
    },
    # 以后可以在这里加新工具，比如：
